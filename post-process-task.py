...
def pid_runs(df: pd.DataFrame):
    ...
    for i, value in enumerate(top_20_process_counts.values):
        ax.barh(
            bar_positions[i], value, color=colors[i % len(colors)], edgecolor="none"
        )

    # Add text labels to the bars
    for i, value in enumerate(top_20_process_counts.values):
        label_x_pos = (
            value + max(top_20_process_counts.values) * 0.01
...
def tid_runtime_cumulative(df: pd.DataFrame) -> None:
    ...
    for i, (index, value) in enumerate(
        zip(cumulative_runtimes.index[::-1], cumulative_runtimes.values[::-1])
    ):
...
def pid_runtime_cumulative(df: pd.DataFrame) -> None:
    ...
    cumulative_runtimes = (
        df.groupby("PID")["Runtime"].sum().sort_values(ascending=False).head(20)
    )
...
def comm_runtime_cumulative(df: pd.DataFrame) -> None:
    cumulative_runtimes = (
        df.groupby("Comm")["Runtime"].sum().sort_values(ascending=False).head(20)
    )
    ...
    ax.get_xaxis().set_major_formatter(
        FuncFormatter(lambda x, pos: f"{x / 1000000:.0f}")
    )
...
def tid_runtime_distribution_violin(df: pd.DataFrame) -> None:
    ...
    for tid in top_20_tids:
        ...
        labels.append(
            f"{tid} ({representative_record['Comm']}, {representative_record['PID']})"
        )
    ...
    mean_marker = plt.Line2D(
        [], [], color="blue", marker="o", linestyle="None", alpha=0.3, label="Mean"
    )
    quartiles_marker = plt.Line2D(
        [],
        [],
        color="blue",
        marker="d",
        linestyle="None",
        alpha=0.3,
        label="Quartiles (first and third)",
    )
    median_line = plt.Line2D(
        [], [], color="mediumpurple", marker="_", linestyle="None", label="Median"
    )
...
def comm_runtime_distribution_violin(df: pd.DataFrame) -> None:
    ...
    data = [
        filtered_df[filtered_df["Comm"] == task]["Runtime"] for task in top_20_tasks
    ]
...
def tid_runtime_distribution_scatter(df: pd.DataFrame) -> None:
    ...
    cumulative_runtimes_by_tid = (
        df.groupby("TID")["Runtime"].sum().sort_values(ascending=False).head(20)
    )
    ...
    for tid in top_20_tids:
        ...
        x = np.random.normal(
            size=len(task_df), loc=np.where(top_20_tids == tid)[0][0] + 1, scale=0.1
        )
        y = task_df["Runtime"]
        ax.scatter(x, y, alpha=0.1, s=25, color="royalblue")

    tid_labels = [
        f'{tid} ({filtered_df.loc[filtered_df["TID"] == tid, "Comm"].iloc[0]}, {filtered_df.loc[filtered_df["TID"] == tid, "PID"].iloc[0]})'
        for tid in top_20_tids
    ]
...
def comm_runtime_distribution_scatter(df: pd.DataFrame) -> None:
    ...
    for task in top_20_tasks:
        x = np.random.normal(
            size=len(filtered_df[filtered_df["Comm"] == task]["Runtime"]),
            loc=top_20_tasks.get_loc(task) + 1,
            scale=0.1,
        )
...
def tid_runtime_spread(df: pd.DataFrame) -> None:
    df["TaskLabel"] = df.apply(
        lambda x: f"{x['TID']} ({x['Comm']}, {x['PID']})", axis=1
    )
    cumulative_runtimes = (
        df.groupby(["TID", "TaskLabel"])["Runtime"].sum().reset_index()
    )
    top_20_tasks = cumulative_runtimes.sort_values(by="Runtime", ascending=False).head(
        20
    )
    spread_ratings_df = pd.DataFrame(
        {
            ...
            "SpreadRating": [
                calculate_spread_rating(df[df["TID"] == row["TID"]]["Runtime"])
                for index, row in top_20_tasks.iterrows()
            ],
        }
    )

    spread_ratings_df_sorted = spread_ratings_df.sort_values(
        by="SpreadRating", ascending=False
    )
    ...
    bars = ax.bar(
        spread_ratings_df_sorted["TaskLabel"],
        spread_ratings_df_sorted["SpreadRating"],
        color="dodgerblue",
    )
...
def comm_runtime_spread(df: pd.DataFrame) -> None:
    ...
    spread_ratings = [
        calculate_spread_rating(df[df["Comm"] == task]["Runtime"])
        for task in top_20_tasks
    ]
...
def tid_runtime_sleeptime_spread(df: pd.DataFrame) -> None:
    ...
    tid_labels["Label"] = tid_labels.apply(
        lambda x: f'{x.name} ({x["Comm"]}, {x["PID"]})', axis=1
    )
    ...
    for tid in top_tids:
        ...
        sleeptime_spreads.append(
            calculate_spread_rating(df[df["TID"] == tid]["Time Out-In"])
        )
...
def comm_runtime_sleeptime_spread(df: pd.DataFrame) -> None:
    ...
    for task in top_tasks:
        runtime_spreads.append(
            calculate_spread_rating(df[df["Comm"] == task]["Runtime"])
        )
        sleeptime_spreads.append(
            calculate_spread_rating(df[df["Comm"] == task]["Time Out-In"])
        )
...
def tid_runtime_linechart(df: pd.DataFrame) -> None:
    ...
    (p1_line,) = ax.plot(
        p1["Switched-In"], p1["Runtime"], marker=None, linestyle="-", color="royalblue"
    )
    (p2_line,) = ax.plot(
        p2["Switched-In"], p2["Runtime"], marker=None, linestyle="-", color="darkorchid"
    )
    (p3_line,) = ax.plot(
        p3["Switched-In"], p3["Runtime"], marker=None, linestyle="-", color="darkorange"
    )
    ...
    ax.text(
        p1["Switched-In"].iloc[-1] + label_offset,
        p1["Runtime"].iloc[-1],
        label1,
        va="center",
        fontsize=12,
    ...
    ax.text(
        p2["Switched-In"].iloc[-1] + label_offset,
        p2["Runtime"].iloc[-1],
        label2,
        va="center",
        fontsize=12,
    ...
    ax.text(
        p3["Switched-In"].iloc[-1] + label_offset,
        p3["Runtime"].iloc[-1],
        label3,
        va="center",
        fontsize=12,
    ...
    ax.scatter(
        p1["Switched-In"], p1["Runtime"], color="royalblue", alpha=0.4, edgecolor="none"
    )
    ax.scatter(
        p2["Switched-In"],
        p2["Runtime"],
    ...
    for i, tid in enumerate(tid_values):
        ...
        if not p.empty:
            ...
            try:
                label_offset = 0.1
                ax.text(
                    p["Switched-In"].iloc[-1] + label_offset,
                    p["Time Out-In"].iloc[-1],
                    label,
                    va="center",
                    fontsize=12,
                )
...
def comm_sleeptime_linechart(df: pd.DataFrame) -> None:
    ...
    (p1_line,) = ax.plot(p1["Switched-In"], p1[key], linestyle="-", color="royalblue")
    (p2_line,) = ax.plot(p2["Switched-In"], p2[key], linestyle="-", color="darkorchid")
    (p3_line,) = ax.plot(p3["Switched-In"], p3[key], linestyle="-", color="darkorange")
...
def tid_runs_vs_runtime_quadrant(df: pd.DataFrame) -> None:
    ...
    task_summary = pd.DataFrame(
        {"Total_Runtime": total_runtime, "Count": execution_count}
    ).join(labels_df)

    fig, ax = plt.subplots(figsize=(12, 12))

    sizes = (
        task_summary["Total_Runtime"] / task_summary["Total_Runtime"].max() * 1000
    )  # Normalize sizes for visibility
    scatter = ax.scatter(
        task_summary["Total_Runtime"], task_summary["Count"], s=sizes, alpha=0.6
    )
    ...
    x_geo_mean = (
        task_summary["Total_Runtime"].max() - task_summary["Total_Runtime"].min()
    ) // 2
...
def comm_runs_vs_runtime_quadrant(df: pd.DataFrame) -> None:
    ...
    x_median = np.sqrt(
        task_summary["Total_Runtime"].min() * task_summary["Total_Runtime"].max()
    )
...
def tid_runtime_utilization(df: pd.DataFrame) -> None:
    ...
    top_tasks = top_runtime_sum.merge(
        df[["TID", "Comm", "PID"]].drop_duplicates(), on="TID", how="left"
    )

    # Creating a label column "<TID> (<Comm>, <PID>)"
    top_tasks["Label"] = top_tasks.apply(
        lambda x: f'{x["TID"]} ({x["Comm"]}, {x["PID"]})', axis=1
    )
    ...
    for _, row in df.iterrows():
        if row["TID"] in top_tasks["TID"].values:
            ...
            label = top_tasks[top_tasks["TID"] == row["TID"]]["Label"].values[
                0
            ]  # Find the label for the TID
    ...
    cpu_utilization = (
        cpu_utilization / 1e6
    )  # Adjusted division for correct percentage calculation
    ...
    for i, label in enumerate(top_tasks["Label"]):
        ...
        axes[i].fill_between(
            cpu_utilization.index, 0, cpu_utilization[label], step="mid", alpha=0.5
        )
...
def comm_runtime_utilization(df: pd.DataFrame) -> None:
    ...
    for i, task in enumerate(total_runtime.index):
        ...
        axes[i].yaxis.grid(
            which="major", linestyle=":", linewidth="0.5", color="#bbbbbb"
        )
        axes[i].yaxis.grid(
            which="minor", linestyle=":", linewidth="0.5", color="#bbbbbb"
        )
        axes[i].xaxis.grid(
            which="major", linestyle=":", linewidth="0.5", color="#bbbbbb"
        )
        ...
        axes[i].fill_between(
            cpu_utilization.index, 0, cpu_utilization[task], step="mid", alpha=0.5
        )
...
def tid_runtime_overall_pie(df: pd.DataFrame) -> None:
    df["Runtime"] /= 1e9
    df["Label"] = (
        df["TID"].astype(str) + " (" + df["Comm"] + ", " + df["PID"].astype(str) + ")"
    )
    ...
    remain_threshold = 3
    remain_runtime = total_runtime_per_label[
        runtime_percentage < remain_threshold
    ].sum()
    filtered_runtime_per_label = total_runtime_per_label[
        runtime_percentage >= remain_threshold
    ]
    ...
    explode = [
        0.1 if label == remain_label else 0
        for label in filtered_runtime_per_label.index
    ]

    fig, ax = plt.subplots(figsize=(14, 7))
    colors = plt.get_cmap("tab10").colors
    ax.pie(
        filtered_runtime_per_label,
        labels=filtered_runtime_per_label.index,
        autopct="%1.1f%%",
        startangle=90,
        counterclock=False,
        colors=colors[: len(filtered_runtime_per_label)],
        explode=explode,
    )
...
def comm_runtime_overall_pie(df: pd.DataFrame) -> None:
    ...
    filtered_runtime_per_comm = total_runtime_per_comm[
        runtime_percentage >= remain_threshold
    ]
    ...
    explode = [
        0.1 if label == remain_label else 0 for label in filtered_runtime_per_comm.index
    ]

    fig, ax = plt.subplots(figsize=(14, 7))
    colors = plt.get_cmap("tab10").colors
    ax.pie(
        filtered_runtime_per_comm,
        labels=filtered_runtime_per_comm.index,
        autopct="%1.1f%%",
        startangle=90,
        counterclock=False,
        colors=colors[: len(filtered_runtime_per_comm)],
        explode=explode,
    )
...
def pid_tid_accumulated_runtime_stacked(df: pd.DataFrame) -> None:
    ...
    top_20_tasks = (
        cumulative_runtimes_seconds.sort_values(ascending=False).head(20).index
    )
    ...
    pivot_df = (
        filtered_df.groupby(["PID", "TID"])["Runtime"].sum().unstack(fill_value=0) / 1e9
    )
    ...
    ax.set_xticklabels(
        [f"{comm_names[pid]} ({pid})" for pid in pivot_df.index],
        rotation=45,
        ha="right",
    )
...
def comm_pid_tid_hierarchy_dot(runtimes, filename, filter) -> None:
    with open(filename, "w") as f:
        ...
        for comm, pids in runtimes.items():
            if filter:
                if not any(
                    comm.lower() in string.lower() or comm.lower() == string.lower()
            ...
            for pid, tid_dict in pids.items():
                ...
                for tid, runtime in tid_dict.items():
                    ...
                    f.write(
                        f'  "{comm}_{pid}_{tid}" [{tid_style}, label="{tid_label}"];\n'
                    )
...
def pid_tid_hierarchy(filter=None) -> None:
    runtimes = defaultdict(
        lambda: {
            "_total": 0,
            "_comm": "",
            "tids": defaultdict(lambda: {"runtime": 0, "comm": ""}),
        }
    )
